...
MsgPack supports additional types including ArrayBuffer types. While it is not JSON, it can be used with Prim+RPC. We'll
add a few additional options to the handler to allow Prim+RPC plugins to better work with binary data:
...
Like the SuperJSON example, devalue may be used to support additional types with Prim+RPC. The JSON handler can be set
up like so:
...
YAML may be used with Prim+RPC and may be a more readable alternative to JSON. The JSON-like handler can be set up like
so:
