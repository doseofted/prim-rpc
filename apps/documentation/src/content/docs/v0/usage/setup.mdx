...
environments so while this guide is interactive in that it allows you to choose your own integrations, it will also try
to remain general in many regards. **Let's get started!**
...
separate folders, a monorepo, or across two repositories using a private registry). If you don't, you can use
[the starter project](https://github.com/doseofted/prim-rpc-examples/tree/main/starter) to follow along.

Th following one-liner will download the starter project:
...
**[Required: Method Handler](#method-handler)**: receives function calls and sends results to client.

<PluginChoices type="method-handler" />

</div>

<div id="method-plugin">

**[Required: Method Plugin](#method-plugin)**: sends function calls and receives results from the server.

<PluginChoices type="method-plugin" />

</div>

You can add support for callbacks and other advanced types with additional plugins.

<Collapse title="Optional Choices">

<div id="callback-handler">

**[Callback Handler](#callback-handler)**: handles callbacks on functions and sends data to the client.

<PluginChoices type="callback-handler" />

</div>

<div id="callback-plugin">

**[Callback Plugin](#callback-plugin)**: receives callback data and fires given callbacks on functions.
...
**[JSON Handler](#json-handler)**: optionally support advanced types like Map/Set.
...
<CodeFile filename="server/example.ts">
...
Notice that last line. We added an `rpc` property to the function with a value of `true`. This signals to Prim+RPC that
it can safely call this function on the server. Functions are objects in JavaScript so if this looks unfamiliar, this is
...
requests into RPC that it can understand. On the server, these are **method handlers** that handle calls to a method and
**callback handlers** that handle callback functions.

**Method Handler Setup**

**Callback Handler Setup**

And now our server is fully configured and ready to work with the client! Let's set up the client next.
