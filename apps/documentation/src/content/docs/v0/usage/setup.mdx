...
<Aside message="If you're using the starter project, you can skip this step: Prim+RPC is already installed in both server and client parts of the project." />
...
<Aside message="You're all set up! Read on to support callbacks and advanced types in your functions." type="success" />

## Support Callbacks

So far, we can already call functions on the server from the client with support for all valid JSON types and even Files
and Blobs. But Prim+RPC has a few other tricks up its sleeve: support for callbacks and many other types.

We'll start with callbacks because it's kind of fun. Or you may skip ahead if you'd like to
[support even more types](#extended-types).

## Extended Types

Prim+RPC uses the built-in JSON handler for its environment by default to serialize RPC messages. This means that we can
pass arguments and receive return values from our function between the server and client, as long as the JSON handler
can serialize it. However this can be limiting because we may want to work with additional types like Dates, Maps, Sets,
and more.

That's why Prim+RPC allows you to swap out this JSON handler with you own. For instance, you may use
[superjson](https://github.com/blitz-js/superjson#readme) to support many JavaScript built-ins or
[devalue](https://github.com/Rich-Harris/devalue#readme) for the same reason (plus support for cyclical references). In
fact, it doesn't even need to be JSON. You may choose to serialize messages using
[YAML](https://github.com/eemeli/yaml#readme) (for readability) or
[MsgPack](https://github.com/msgpack/msgpack-javascript#readme) (for its size and support for more types).
