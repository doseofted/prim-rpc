...
This function takes either a string of Markdown or a Markdown file and converts it into an HTML file. We can call this
function from the client like so:
...
There are many features available in Prim+RPC that we haven't even touched yet. You could (and should) validate
arguments inside of your functions (using any validation library). You may define methods on other methods or swap out
your server framework. See the [usage guide](/docs/v0/learn/usage) or one of the
[available examples](/docs/v0/learn/examples) to learn more about what can be done with Prim+RPC.
