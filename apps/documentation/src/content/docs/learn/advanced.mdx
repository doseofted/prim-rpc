...
We can use any validation framework that we like but we'll a popular framework, Zod. Let's install it on the server:
...
```typescript {1,4-5} /x: string/ /y: string/
import { z } from "zod"
...
Now we can ensure that the our arguments are strings. This works well for one function but if we have a lot of
functions, we may want to require that all functions have their arguments validated and fail if they are not.

We can easily add this functionality to Prim+RPC using its `preprocess` option. This will allow us to modify our
function arguments prior to passing them to the intended function. Let's set this up:

<Aside type="error">

**Todo:** add preprocess option to Prim+RPC
...
function contextTransform(req: Request, res: Response) {
