...
| ---------------------------------------------- | -------------- | -------------- | ------------ |
| [Fetch API](https://github.com/wintercg/fetch) | Method handler | HTTP           | `File`       |

Prim+RPC supports the Fetch API on the server. This can be used with any runtime: Node, Deno, or Bun. The plugin accepts
a standard `Request` and returns a standard `Response`.

You can configure the handler in the following ways (depending on your environment). In Bun:
...
// Using generic default fetch export
export default primFetch({ prim })
// Or otherwise:
Bun.serve({ fetch: primFetch({ prim }) })
```

In Deno:

```typescript index.ts
// NOTE: specify version, not latest, this is only an example
import { createPrimServer } from "npm:@doseofted/prim-rpc@latest"
import { primFetch } from "npm:@doseofted/prim-rpc-plugins@latest/server-fetch"
import module from "./my-example-module"

const prim = createPrimServer({ module })

Deno.serve(primFetch({ prim }))
```

In Node, using an adapter to allow usage of Fetch API:
...
The Request and Response can be manipulated using the `postprocess` and `preprocess` option of the plugin respectively.
