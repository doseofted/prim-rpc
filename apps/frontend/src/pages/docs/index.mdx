...
server. Prim+RPC doesn't concern itself with the method of delivery and instead allows you to specify a handler plugin
to send RPC. In this example, we'll use a handler for a server framework called Fastify but there are many other
handlers available and it is easy to write your own.
...
The client that we'll create can automatically generate this RPC (the `--data` parameter above). In a new file, we'll
create a Prim+RPC client that will communicate with our newly configured server.
...
JavaScript, we could've also used a JSDoc comment to annotate our client. These types are totally optional and the
client has no dependency on them (for those who prefer JavaScript). Also note that we are _only_ importing types: code
from the server is not imported. Instead, this will be requested over the endpoint that we've configured.

By default, the Prim+RPC does not assume a default client. Similar to the server, we provide this functionality through
the use of plugins. Let's add support for our server using the browser's built-in fetch client (supported with a
Prim+RPC plugin for the browser's APIs):
...
This plugin will use the browser's Fetch API so call our function given at the endpoint we've given. We also have the
option of configuring this plugin or setting up our own client if we'd like (it doesn't even need to be an HTTP client).
Now we're ready to call our function from the client to our server!
...
This introduction has only shown the basic RPC features available with Prim+RPC. There are many additional features like
callback support and file upload handling, options that you can configure, plugins that you can utilize (or create your
own), serialization handlers for RPC that you can customize, documentation that you can generate, and more.

You can follow guides on this website or [see the available examples](/docs/examples) to get started!
...
source. The availability of these tools are a big part of why I am a developer today and that ecosystem is something for
which I hope that I can contribute back. I hope that you find this library useful and that it helps other developers
like yourself whether you are experienced or just starting out.

If you do find this tool useful, consider giving it a star on Github, sharing with others, contributing back to the
project (whether to the core project or a plugin for it), sponsoring the project, or by working with Ted through his
company, Dose of Ted.
