...
Fastify as an example). The second example however will log `undefined` instead of the expected `"hello!"` because
Prim+RPC set the `this` value to another value, where `.someProperty` doesn't exist.
