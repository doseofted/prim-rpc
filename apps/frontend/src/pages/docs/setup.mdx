...
can download an example project to follow along. The command below will download the example project or you can download
the it [directly from GitHub](https://github.com/doseofted/prim-rpc-examples/tree/main/starter):

```bash
npx giget@latest gh:doseofted/prim-rpc-examples/starter
```
...
If you'd like to get started using Prim+RPC in a pre-configured project, you may reference
...
Now we can set up the Fastify-specific "method handler" plugin which will handle method calls (so we can call the
`sayHello()` method on the Prim+RPC client).
...
[many plugins available](/docs/plugins/client) for your client. For this example, we'll use a plugin for the Fetch API
since it's built into the web browser.
...
Let's go ahead and make a remote function call!
...
plugin on the client. Follow the [Server Setup](#server-setup) [Client Setup](#client-setup) guides first before
...
connections on the client. We'll use Prim+RPC's callback plugin for the browser which uses the WebSocket API.
