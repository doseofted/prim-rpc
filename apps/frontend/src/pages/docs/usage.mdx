...
export function uploadFile(someTextFile: Promise<string>) {
...
client.uploadFile(file) // server will log contents of file
```
...
export function submitContactForm(inputs: FormInputs) {
...
If you're using TypeScript, you can define multiple signatures on your function and those types will be available on the
client.
...
You can use nested modules with Prim+RPC.
