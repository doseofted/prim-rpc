...
[write your own](/docs/plugins/server/create) if a server handler doesn't meet your needs or you need support for an
additional server. An example of how to use an existing method handler is demonstrated below:
...
createPrimClient({
	endpoint: "http://some-server.localhost:1234/my-rpc-endpoint",
	// ... other options
})
```
...
```zsh
curl --request GET \
  --url http://some-server.localhost:1234/my-rpc-endpoint/sayHello?name=Ted&greeting=Hello
```
