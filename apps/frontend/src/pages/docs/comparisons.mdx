...
tested a few other popular schema libraries and allows them to be used with tRPC. Prim+RPC on the other hand only
focuses on compile-time validation of types and leaves the run-time errors to the developer. This means that you could
potentially use Zod, jut like you would in tRPC, but you also have the option of choosing any other tool from the
JavaScript ecosystem if you'd like.
...
| Mostly sses Zod for schema validation | Schema validation is left to developer but Zod could be used |
