...
that framework's data loaders (where a Prim+RPC instance serves a dedicated role for an API).
...
(and the server can more easily be swapped in the future) whereas with REST endpoints in Qwik or Solid Start you need to
use middleware or plugins specific to that framework.
