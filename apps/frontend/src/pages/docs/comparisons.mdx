...
many different ways that makes discussion around these topics abstract and difficult to define, especially in a way that
reflects how they're actually used today. So instead we'll focus on specific tooling that implements these styles since
it is easier to make direct comparisons against concrete features.
...
run-time errors to the developer. This means that you could potentially use Zod, jut like you would in tRPC, but you
...
| tRPC                                  | Prim+RPC                                                     |
| ------------------------------------- | ------------------------------------------------------------ |
| Focus on HTTP transport               | Transport-agnostic (provided over plugins)                   |
...
Prim+RPC and gRPC are very different which makes it difficult to accurately compare both without leaving something out
or oversimplifying. However, we'll try to make some general comparisons.
