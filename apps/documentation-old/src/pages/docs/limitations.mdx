...
objects. However, this is subject to the limitations of the default JSON handler unless overridden. This means that
other types of data will need to be serialized to a string and will not automatically convert back to the same type on
the server. If the data type passed to Prim+RPC is not something that can be serialized then it may not be sent at all.
