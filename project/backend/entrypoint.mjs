...
const prefix = await gatherPackageName("project/backend/package.json")
...
if (mode !== "production") {
	...
	dev = nothrow($`pnpm dev --filter="@doseofted/prim-backend"`) // wrap in `nothrow` since it's just a dev process
}

const args = process.argv.slice(3).join(" ")
echo`Starting app in ${mode} mode ...`
try {
	const app = $`pnpm ${args || (mode === "production" ? "start" : "watchstart")} --filter="@doseofted/prim-backend"`
