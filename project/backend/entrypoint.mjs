...
if (mode !== "production") {
	...
	dev = nothrow($`pnpm --filter="@doseofted/prim-backend" dev`) // wrap in `nothrow` since it's just a dev process
}

const args = process.argv.slice(3).join(" ")
echo`Starting app in ${mode} mode ...`
try {
	const app = $`pnpm --filter="@doseofted/prim-backend" ${args || (mode === "production" ? "start" : "watchstart")}`
