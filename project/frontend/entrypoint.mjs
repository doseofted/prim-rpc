...
if (mode !== "production") {
	...
	dev = nothrow($`pnpm dev --filter="@doseofted/prim-frontend"`) // wrap in `nothrow` since it's just a dev process
}
...
try {
	...
	const app = new Promise(r => r()) // $`pnpm ${args || "preview"} --filter="@doseofted/prim-frontend"`
