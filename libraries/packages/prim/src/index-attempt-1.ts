...
interface RpcCall<T = unknown, M = string> extends RpcBase {
	method: M
	params?: T
}
...
// Record<string, (...a: Parameters<T[V]>[]) => ReturnType<T[V]>>
// function (rpcCall: RpcCall<Parameters<T[V]>[], MethodNames>): RpcAnswer<ReturnType<T[V]>>
/**
 ...
 * Prim.
 */
export function prim<T extends Record<keyof T, T[V]>, V extends keyof T>(givenModule: T) {
	const givenNames = Object.keys(givenModule)
	console.log(givenNames)
	return function (rpcCall: RpcCall<Parameters<T[V]>, V>): RpcAnswer<ReturnType<T[V]>> {
...
const a = {
	yo: (a: string, b: string) => {
		console.log(a, b); return { c: a + b }
	},
	hello: (name: { first: string, last: string }) => `Hello ${name.first}`
}
const t = prim(a)
const r = t({ jsonrpc: "2.0", method: "hello", params: [{ first: "", last: ""}], id: null })
...
/**
 * A version of `prim` function that accepts given paramters from common server frameworks.
 * Essentially a generic wrapper around Prim specifically for middleware/plugins to be written.
 */
// eslint-disable-next-line @typescript-eslint/no-unused-vars
export function primHttp (given: PrimHttp) {
	// TODO: add to this function some of the logic from the Fastify plugin in this project, under backend
}
