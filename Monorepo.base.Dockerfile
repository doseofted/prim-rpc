FROM node:19.0-bullseye as monorepo-install
USER root
...
RUN corepack enable
RUN corepack prepare pnpm@7.14.1 --activate
...
COPY --chown=node package.json .eslint* .nvmrc Taskfile.yml tsconfig.json turbo.json build-deps.mjs uno.config.mjs  ./
