...
You can use Prim+RPC with Next.js by defining a dedicated route.

<Alert icon="carbon:information" type="alert-info">
	Currently, only the App Router introduced in Next.js 13 is supported but support for the Pages Router is planned.
</Alert>

You can configure the integration on the server like so (note that names of files are important):
...
On the client-side, you can use the Prim+RPC client just like any other integration using a compatible method plugin.
Since the client could be used on either the server or client, you may conditional set the URL depending on the
environment, like so:
