...
export const createFetchClient = (headersOverride?: Headers|Record<string, string>) => {
	const primClient: PrimClientFunction = async (endpoint, jsonBody, jsonHandler, blobs = {}) => {
		...
		if (blobList.length > 0) { // send as form data if blobs is given (empty if not configured)
		...
		} else { // send all RPC requests without blobs as regular JSON
			fetchOptions = {
				method: "POST",
