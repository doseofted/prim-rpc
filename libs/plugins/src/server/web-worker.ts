...
export const primMethodWebWorker = (options: MethodWebWorkerOptions): PrimServerMethodHandler => {
	...
	return prim => {
		...
		worker.addEventListener("message", async (event) => {
			...
			let { data } = event
			if (useJsonHandler && typeof data === "string") { // Prim Client option was set to same value as server
				// eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
				data = jsonHandler.parse(data)
			} else if (useJsonHandler) { // Prim Client doesn't know server is using JSON handler (consider throwing error)
				// eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
...
export const primCallbackWebWorker = (options: MethodWebWorkerOptions): PrimServerCallbackHandler => {
	...
	return prim => {
		...
		worker.addEventListener("messageerror", () => { ended() })
	}
}
