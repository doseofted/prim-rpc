...
test("Basic documentation structure can be generated", () => {	
	...
	function docsStructureCorrect (rootDocs: PrimRpcDocs, struct?: Partial<PrimRpcDocs>, level = 0): boolean {
		struct ??= docs
		const root = level === 0
		const docsRefCorrect = struct.docs
		...
		const propsCorrect = propsNotGiven || (typeof struct.props === "object"
		...
		const methodsCorrect = struct.methods?.map(method => typeof method.name === "string"
			&& typeof method.path === "string"
			&& method.signatures.length > 0).reduce((p, n) => p && n, true) ?? false
		const modulesCorrect = struct.modules?.map(module => typeof module.name === "string"
			&& typeof module.path === "string").reduce((p, n) => p && n, true) ?? false
