...
if (inDevelopment) {
	...
	watch(libsPath, { ignored: /node_modules/, ignoreInitial: true }).on("all", async (_e, p) => {
		...
		const createProc = () => {
			const proc = $`pnpm task dev:build -- --filter=${filter}`.nothrow()
			void (async () => {
				await proc
				delete processes[projectPath]
			})()
			return proc
		}
